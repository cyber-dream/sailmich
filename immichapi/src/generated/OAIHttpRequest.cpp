...
OAIHttpRequestWorker::OAIHttpRequestWorker(QObject *parent, QNetworkAccessManager *_manager)
    ...
    randomGenerator = QRandomGenerator(QDateTime::currentDateTime().toTime_t());
    if (manager == nullptr) {
...
void OAIHttpRequestWorker::execute(OAIHttpRequestInput *input) {
    ...
    if (input->http_method == "GET") {
        ...
        buffer->setData(request_content); // или httpContent, смотря как названа переменная
        buffer->open(QIODevice::ReadOnly);

        // 2. Отправляем запрос, передавая буфер как QIODevice*
        reply = manager->sendCustomRequest(request, input->http_method.toLatin1(), buffer);

        // 3. Важно! Привязываем удаление буфера к завершению ответа, чтобы не было утечки памяти
...
void OAIHttpRequestWorker::process_response(QNetworkReply *reply) {
    ...
    if (!contentDispositionHdr.isEmpty()) {
        auto contentDisposition = contentDispositionHdr.split(QString(";"), QString::SkipEmptyParts);
        auto contentType =
            !contentTypeHdr.isEmpty() ? contentTypeHdr.split(QString(";"), QString::SkipEmptyParts).first() : QString();
        QString dispositionType = contentDisposition.first().trimmed();
//        qInfo() << dispositionType;
        if ((contentDisposition.count() > 0) &&
                (dispositionType.compare("attachment", Qt::CaseInsensitive) == 0 ||
                     dispositionType.compare("inline", Qt::CaseInsensitive) == 0)){ // add "inline" check for immich thumbnails response
        ...
        auto contentType = contentTypeHdr.split(QString(";"), QString::SkipEmptyParts);
        if ((contentType.count() > 0) && (contentType.first() == QString("multipart/form-data"))) {
            ...
            if(!contentEncodingHdr.isEmpty()){
                auto encoding = contentEncodingHdr.split(QString(";"), QString::SkipEmptyParts);
                if(encoding.count() > 0){
