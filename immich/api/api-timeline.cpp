...
Immich::ResultErrored<QVector<TimeBucketAsset>>
ApiTimeline::getBucket(const QDate &pDate,
                       const GetBucketQueryParam &params) const {
  if (!pDate.isValid())
    return {ResultError("date parameter is invalid")};
  QUrlQuery query;
  ...
  if (rep->error())
    return {ResultError(rep->errorString())};

  QByteArray ReplyText = rep->readAll();
  QJsonDocument doc = QJsonDocument::fromJson(ReplyText);
  if (!doc.isObject())
    return {ResultError("json is not an object")};

  const auto obj = doc.object();

  if (!obj["id"].isArray())
    return {ResultError("malformed json object")};
  ...
  for (int idx = 0; idx < bucketSize; ++idx) {
    ...
    if (!assets[idx].isValid())
      return {ResultError("can't parse timebucket [" +
                          QVariant(idx).toString() +
                          "]: " + assets[idx].errMessage())};
  }

  return ResultErrored<QVector<TimeBucketAsset>>(assets);
}
...
ApiTimeline::getBuckets(const GetBucketsQueryParam &params) const {
  ...
  QJsonDocument doc = QJsonDocument::fromJson(ReplyText);
  if (!doc.isArray())
    return {ResultError("json is not an array")};
  ...
  for (int idx = 0; idx < array.size(); ++idx) {
    if (!array[idx].isObject())
      return {ResultError("can't parse timelineBuckets, [" +
