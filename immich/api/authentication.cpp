...
Immich::ResultErrored<ValidateTokenResponse>
AuthenticationApi::validateToken() const {
  ...
  if (!validateTokenResponse.isValid())
    return {ResultError("validateTokenResponse is not valid:" +
                        validateTokenResponse.errMessage())};

  return {validateTokenResponse};
}

Immich::ResultErrored<LoginResponse>
AuthenticationApi::login(const QUrl &pAddress, const QString &pEmail,
  ...
  if (!loginResponse.isValid())
    return {ResultError("loginResponse is not valid: " +
                        loginResponse.errMessage())};

  return {loginResponse};
}

Immich::ResultErrored<LogoutResponse> AuthenticationApi::logout() const {
  ...
  if (logoutResponse.isValid()) {
    return {ResultError("logoutResponse is not valid:" +
